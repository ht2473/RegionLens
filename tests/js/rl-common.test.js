...
describe("RL.normalizeWeights — нормировка весов доменов в проценты", () => {
  const KEYS = ["a", "b", "c", "d"];

  it("равные веса дают равные доли, сумма = 100%", () => {
    const out = window.RL.normalizeWeights({ a: 1, b: 1, c: 1, d: 1 }, KEYS);
    KEYS.forEach((k) => expect(out[k]).toBeCloseTo(25));
    const sum = KEYS.reduce((s, k) => s + out[k], 0);
    expect(sum).toBeCloseTo(100);
  });

  it("пропорциональны значениям ползунков", () => {
    const out = window.RL.normalizeWeights({ a: 3, b: 1, c: 0, d: 0 }, KEYS);
    expect(out.a).toBeCloseTo(75);
    expect(out.b).toBeCloseTo(25);
    expect(out.c).toBeCloseTo(0);
  });

  it("все веса нулевые → равные доли (без деления на ноль)", () => {
    const out = window.RL.normalizeWeights({ a: 0, b: 0, c: 0, d: 0 }, KEYS);
    KEYS.forEach((k) => expect(out[k]).toBeCloseTo(25));
  });

  it("отрицательные веса трактуются как ноль", () => {
    const out = window.RL.normalizeWeights({ a: -5, b: 2, c: 2, d: 0 }, KEYS);
    expect(out.a).toBeCloseTo(0);
    expect(out.b).toBeCloseTo(50);
    expect(out.c).toBeCloseTo(50);
  });
});
