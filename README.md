...
- **Пространственная автокорреляция**: глобальный Moran's I и локальный LISA (карта кластеров HH/LL/HL/LH) — похожи ли соседние регионы по индексу.
...
Основные стадии: ETL (справочники и факты) → признаки (ядро, импутация, z-score) → типология (кластеры + SHAP) → индекс развития (3 схемы весов) → переходы и траектории типов → статистические двойники → стабильность рейтинга → дисперсия и σ-сходимость → β-сходимость → пространственная автокорреляция (Moran's I / LISA) → парные корреляции → декомпозиция индекса → аномалии → качество данных.
...
Публичные страницы: главная, карта, explore-режим, рейтинги, лаборатория индекса, сходимость, стабильность рейтинга, типология, сравнение регионов, список и карточка региона, методология, данные, качество данных, неравенство/дисперсия, аномалии, корреляции, пространственная автокорреляция, справка.
...
Свежие parquet-выгрузки коллекции Росстата (портал [«Если быть точным»](https://tochno.st/datasets/regions_collection)) принимаются одной командой. Она проверяет схему файла (14 канонических колонок), пересобирает всю аналитику в отдельный staging-файл — боевая витрина в это время продолжает обслуживать чтение, — независимо валидирует результат (ключевые таблицы непусты, число регионов в санитарном диапазоне, `MAX(year)` не откатился), сохраняет резервную копию и атомарно подменяет витрину. Неудача на любом шаге оставляет боевой файл ровно в прежнем состоянии; работающие воркеры gunicorn подхватывают новую витрину без рестарта (горячее переподключение по сигнатуре файла).

```bash
make refresh SRC=путь/к/data_regions_collection_102_vНОВАЯ.parquet
# эквивалент: python backend/manage.py refresh_data --source путь/к/файлу.parquet
# пересборка из текущего сырья без нового файла: refresh_data --skip-ingest
```

Резервные копии — в `data/backups/` (хранится 3 последних, настраивается `--keep-backups`); откат — вернуть нужный бэкап на место обычным копированием, воркеры подхватят подмену сами. На VPS удобен «каталог приёма»: выгрузка заливается по scp в `data/incoming/`, systemd path-юнит (`deploy/systemd/`) замечает `*.parquet` и запускает обновление автоматически, обработанный файл переезжает в `data/incoming/processed/`. Журнал: `journalctl -u regionlens-refresh.service`. Для обновления нужны зависимости конвейера (`pip install -e ".[pipeline,backend]"`).
